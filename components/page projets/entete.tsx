...
export default function Entete({props}: {props:{projets: any[], outil: any[], outils: any[], etat: any[], users: any[], setPage: (page: string) => void}}) {
    ...
    return (
		<Card className="bg-gradient-to-r from-blue-500 to-purple-500 text-white p-4 rounded-lg mb-4 grid grid-cols-3 shadow-sl gap-10">
			<div className='grow relative top-0 z-10 col-span-3 xl:col-span-2'>
                ...
                <Button variant="link" className="absolute bottom-[-6.02] right-0 text-blue-600 hover:text-blue-700 hover:underline underline-offset-0 hover:cursor-pointer" onClick={() => setPage("a-propos")}>En savoir plus ...</Button>
            </div>
			<div className="bg-gray-100 text-gray-900 p-4 rounded-lg pl-5 pr-5 min-w-auto shadow-xl max-h-min col-span-3 xl:col-span-1 min-w-max">
				<pre>Données :<br/>
					...
					{outil.length} outil{tools.pluralize(outil.length)} de programmation{tools.pluralize(outil.length)}<br/>
					{outils.length} jointure{tools.pluralize(outils.length)} d'outil{tools.pluralize(outils.length)}<br/>
